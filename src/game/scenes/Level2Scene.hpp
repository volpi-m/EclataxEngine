...
namespace Scenes {
    ...
    class Level2Scene : public AScene {
        ...
        public:
            ...
            Level2Scene(const std::string &name, std::shared_ptr<Module::EntityComponentSystem> &ECS, int player);

            /// \param name : the new name of the current scene
            /// \param ECS : ECS instance to pass to the scene
            /// \param players : number of players
            /// \brief scene constructor
            Level2Scene(const char *name, std::shared_ptr<Module::EntityComponentSystem> &ECS, int player);

            /// \param name : the new name of the current scene
            /// \param players : number of players
            /// \brief scene constructor
            Level2Scene(int player, const std::string &name = default_tag);

            /// \param name : the new name of the current scene
            /// \param players : number of players
            /// \brief scene constructor
            Level2Scene(int player, const char *name = default_tag);
        ...
        private:
            ...
            /*! waves instanciation */
            std::vector<std::function<void(Level2Scene &)>> _waves;
